...
var _node_modulesD3 = require('../node_modules/d3');
...
var BrokenChart = (function () {
    function BrokenChart(options) {
        _classCallCheck(this, BrokenChart);

        this.options = _underscore2['default'].extend({ width: 600, height: 400 }, options);
        this.svg = d3.select('.svg-container').append('svg').attr('width', this.options.width).attr('height', this.options.height);
    }

    BrokenChart.prototype.render = function render(renderers) {
        return new Promise(function (resolve, reject) {
            Promise.all(renderers).then(function (data) {
                resolve();
            }, function (e) {
    ...
    BrokenChart.getGridRowIndex = function getGridRowIndex(y, cellSize, length) {
    ...
    BrokenChart.getGridColIndex = function getGridColIndex(x, cellSize, length) {
    ...
    BrokenChart.isPointInRect = function isPointInRect(point, rect) {
        return point.x > rect.x && point.x < rect.x + rect.width && point.y > rect.y && point.y < rect.y + rect.height;
    };

    BrokenChart.getDistance = function getDistance(x1, x2, y1, y2) {
...
var _libsFilerFiler = require('./libs/filer/filer');

var _libsFilerFiler2 = _interopRequireDefault(_libsFilerFiler);

var _underscore = require('underscore');

var _underscore2 = _interopRequireDefault(_underscore);

var CACHE_DIRECTORY = 'jsons';
...
var Cache = (function () {
	...
	Cache.prototype.onError = function onError(e) {
	...
	Cache.prototype.init = function init() {
		...
		var executor = function executor(resolve, reject) {
			_this.filer.init({ persistent: false, size: 1024 * 1024 }, function (fs) {
				_this.createDirectory().then(function (dirEntry) {
	...
	Cache.prototype.removeDirectory = function removeDirectory() {
		var _this2 = this;

		var executor = function executor(resolve, reject) {
			_this2.readDirectory().then(function (data) {

				// Remove.
				var path = '/' + CACHE_DIRECTORY;
				var fsURL = _this2.filer.pathToFilesystemURL(path);
				_this2.filer.rm(fsURL, function () {
					return resolve();
				}, function (e) {
					_this2.onError(e);
					reject(e.name);
	...
	Cache.prototype.readDirectory = function readDirectory() {
		var _this3 = this;

		var executor = function executor(resolve, reject) {
			_this3.filer.ls(CACHE_DIRECTORY, function (entries) {
	...
	Cache.prototype.createDirectory = function createDirectory() {
		var _this4 = this;

		var executor = function executor(resolve, reject) {
			_this4.filer.mkdir(CACHE_DIRECTORY, false, function (dirEntry) {
	...
	Cache.prototype.parseJSON = function parseJSON(fileEntry) {
		var _this5 = this;

		var executor = function executor(resolve, reject) {

			_this5.filer.open(fileEntry, function (file) {

				// Use FileReader to read file.
				var reader = new FileReader();
				reader.onload = function (e) {
					var grid = JSON.parse(e.target.result);
					resolve(grid);
				};
	...
	Cache.prototype.findFile = function findFile(fileName) {
		var _this6 = this;

		fileName = fileName + '.json';
		var executor = function executor(resolve, reject) {
			_this6.readDirectory().then(function (entries) {
				var foundFileEntry = _underscore2['default'].findWhere(entries, { name: fileName });
				if (foundFileEntry) {
					_this6.parseJSON(foundFileEntry).then(function (data) {
						return resolve(data);
					});
				} else {
	...
	Cache.prototype.writeFile = function writeFile(object, cellSize) {
		var json = JSON.stringify(object),
		    name = Cache.generateNameByCellSize(cellSize);

		this.filer.write('/jsons/' + name + '.json', { data: json, type: 'text/json' }, function (fileEntry, fileWriter) {
			return console.log('Save file ' + name);
		}, this.onError);
	};

	Cache.generateNameByCellSize = function generateNameByCellSize(size) {
		return 'grid_' + size;
	};
...
var _renderersPolyFish = require('./renderers/polyFish');

var _cache = require('./cache');

_jquery2['default'](document).ready(function () {

    var $render = _jquery2['default']('.render'),
        $clear = _jquery2['default']('.clear'),
        $svgContainer = _jquery2['default']('.svg-container'),
        $loader = _jquery2['default']('.loader');
...
var Util = {
   ...
   */
  strToDataURL: function strToDataURL(str, contentType, opt_isBinary) {
   ...
   */
  strToObjectURL: function strToObjectURL(binStr, opt_contentType) {
   ...
   * @return {string} A new blob: URL.
   */
  fileToObjectURL: function fileToObjectURL(blob) {
    return self.URL.createObjectURL(blob);
  },
   ...
   */
  fileToArrayBuffer: function fileToArrayBuffer(blob, callback, opt_errorCallback) {
    var reader = new FileReader();
    reader.onload = function (e) {
   ...
   * @return {Blob} A blob representing the array buffer data.
   */
  arrayBufferToBlob: function arrayBufferToBlob(buffer, opt_contentType) {
   ...
   * @param {Function=} opt_error Optional error callback if the read fails.
   */
  arrayBufferToBinaryString: function arrayBufferToBinaryString(buffer, callback, opt_errorCallback) {
    var reader = new FileReader();
    reader.onload = function (e) {
      callback(e.target.result);
    };
   ...
   */
  arrayToBinaryString: function arrayToBinaryString(bytes) {
...
var MyFileError = function MyFileError(obj) {
...
var Filer = new function () {
  ...
  var isFsURL_ = function isFsURL_(path) {
  ...
  var pathToFsURL_ = function pathToFsURL_(path) {
    if (!isFsURL_(path)) {
      if (path[0] == '/') {
   ...
   */
  var getEntry_ = function getEntry_(callback, var_args) {
   ...
   */
  var copyOrMove_ = function copyOrMove_(src, dest, opt_newName, opt_successCallback, opt_errorHandler, opt_deleteOrig) {
    var self = this;
  ...
  Filer.prototype = Object.defineProperties({}, {
    fs: {
      get: function get() {
        return fs_;
      },
  ...
  Filer.prototype.ls = function (dirEntryOrPath, successCallback, opt_errorHandler) {
    ...
    var callback = function callback(dirEntry) {
  ...
  Filer.prototype.mkdir = function (path, opt_exclusive, opt_successCallback, opt_errorHandler) {
    ...
    var createDir = function createDir(rootDir, folders) {
      ...
      rootDir.getDirectory(folders[0], { create: true, exclusive: exclusive }, function (dirEntry) {
  ...
  Filer.prototype.rm = function (entryOrPath, successCallback, opt_errorHandler) {
    ...
    var removeIt = function removeIt(entry) {
      if (entry.isFile) {
  ...
  Filer.prototype.write = function (entryOrPath, dataObj, opt_successCallback, opt_errorHandler) {
    ...
    var writeFile_ = function writeFile_(fileEntry) {
  ...
  Filer.prototype.df = function (successCallback, opt_errorHandler) {
    var queryCallback = function queryCallback(byteUsed, byteCap) {
...
var Pixels = (function () {
    ...
    Pixels.prototype.initCache = function initCache() {
        var _this = this;

        var excecutor = function excecutor(resolve, reject) {

            _this.cache.init().then(function (data) {
                resolve(_this);
            }, function (e) {
                return reject(e);
            });
    ...
    Pixels.prototype.generate = function generate(options) {
        var _this2 = this;

        var _options = options || {};
        _options = _underscore2['default'].extend({
            x: 0,
            y: 0,
            size: 2,
        ...
        var reBuild = function reBuild() {
            ...
            var totalRows = Math.ceil(height / size),
            ...
            while (r < totalRows) {
                ...
                while (c < totalCols) {
                    ...
                    cell.imageData = _this2.context.getImageData(cell.x, cell.y, 1, 1);
            ...
            var pixelGrid = { x: startX, y: startY, gridSize: { w: width, h: height }, size: size, cells: cells, grid: grid };

            // Cache the data using the FS API.
            if (_options.cache) {
                _this2.cache.writeFile(pixelGrid, pixelGrid.size);
            }
        ...
        var excecutor = function excecutor(resolve, reject) {
            if (_options.cache) {
                (function () {
                    var fileName = _cache.Cache.generateNameByCellSize(_options.size);
                    _this2.cache.findFile(fileName).then(function (grid) {
                        resolve(grid);
                    }, function () {
    ...
    Pixels.prototype.loadImage = function loadImage(imagePath) {
        var _this3 = this;
        ...
        return new Promise(function (resolve, reject) {
            ...
            imageObj.onload = function (evt) {
                ...
                _this3.width = context.canvas.width;
                _this3.height = context.canvas.height;
    ...
    Pixels.generateSubGrid = function generateSubGrid(rect, data) {
        var cells = data.cells;
        var subCells = _underscore2['default'].filter(cells, function (obj) {
            var point = { x: obj.x, y: obj.y };
            return _brokenChart.BrokenChart.isPointInRect(point, rect);
        });

        var sortedCells = _underscore2['default'].sortBy(subCells, function (cell) {
            return cell.row;
        }),
            grids = [],
...
var PolyFish = (function (_RendererBase) {
    ...
    function PolyFish(controller) {
        _classCallCheck(this, PolyFish);

        _RendererBase.call(this);
        this.controller = controller;
        this.resolve = function () {};
        this.pixels = new _pixels.Pixels();
    }

    PolyFish.prototype.run = function run() {
        var _this = this;

        var excecutor = function excecutor(resolve, reject) {
            Promise.all([_this.pixels.initCache(), _this.pixels.loadImage('images/blueFish.jpeg')]).then(function () {
                return _this.pixels.generate({ x: 0, y: 0, size: 2, w: 552, h: 394 });
            }).then(function (data) {
                _this._render(data);
                resolve();
            }, function (e) {
    ...
    PolyFish.prototype._render = function _render(data) {
        ...
        for (i = 0; i < 3000; ++i) {
            ...
            var rowIndex = _brokenChart.BrokenChart.getGridRowIndex(y, size, data.grid.length),
                colIndex = _brokenChart.BrokenChart.getGridColIndex(x, size, data.grid[0].length);
            cell = data.grid[rowIndex][colIndex];
            cells.push(cell);
        }

        this.controller.svg.attr('width', data.gridSize.w).attr('height', data.gridSize.h);
    ...
    PolyFish.prototype._layer1 = function _layer1(cells) {
        // D3 Voronoi layer
        ...
        var trianglevoronoi = d3.voronoi().x(function (d) {
            return d.x;
        }).y(function (d) {
            return d.y;
        });
        ...
        var triangles = this.controller.svg.append('g').attr('class', 'renderer-group').append('g').selectAll('path');

        triangles = triangles.data(triangleData);

        triangles.enter().append('path').attr('d', function (d) {

            return lineFunction([{ x: d[0].x, y: d[0].y }, { x: d[1].x, y: d[1].y }, { x: d[2].x, y: d[2].y }]);
    ...
    PolyFish.prototype._layer2 = function _layer2(cells) {
        ...
        var group = svg.append('g').attr('class', 'renderer-group');

        group.selectAll('circle').data(cells).enter().append('circle').attr('class', 'circle').attr('r', function (d) {
            return 1;
        }).attr('cx', function (d) {
            return d.x;
        }).attr('cy', function (d) {
    ...
    PolyFish.size = function size() {
        return { width: 500, height: 600 };
    };

    return PolyFish;
})(_rendererBase.RendererBase);
